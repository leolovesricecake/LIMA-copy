...
from .transforms import design_fourier
...
def fit_fourier_lasso(
    *,
    ...
    for degree in degrees:
        ...
        X_train = design_fourier(train_masks, terms)
        X_val = design_fourier(validation_masks, terms)
        for alpha in alphas:
    ...
    X_test = design_fourier(test_masks, best["terms"])
    pred = best["model"].predict(X_test)
    selected = [
        int(term) for term, coef in zip(best["terms"], best["model"].coef_) if abs(float(coef)) > 1e-9
    ]
    return {
        "method": "fourier_lasso",
        "status": "ok",
